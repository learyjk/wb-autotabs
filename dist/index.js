(() => {
  ...
  var init4 = () => {
    ...
    let currentIndex = parseInt(tabsComponent.querySelector(CURRENT_CLASS)?.getAttribute("data-w-tab")?.slice(-1) || "1", 10) - 1;
    ...
    const animateLoader = (loader, duration) => {
      let width = getComputedStyle(loader).width;
      let height = getComputedStyle(loader).height;
      if (width === "0px") {
        gsapWithCSS.fromTo(loader, { width: "0%" }, {
          width: "100%",
          ease: "none",
          duration,
          onComplete: () => {
            gsapWithCSS.set(loader, { width: "0%" });
          }
        });
      }
      if (height === "0px") {
        gsapWithCSS.fromTo(loader, { height: "0%" }, {
          height: "100%",
          ease: "none",
          duration,
          onComplete: () => {
            gsapWithCSS.set(loader, { height: "0%" });
          }
        });
      }
    };
